...
% AirProperties calculates a variety of air properties from the measured 
...
%   xlabel('Temperature [°C]'); ylabel('\rho [kg m^{-3}]'); title('Density of air')
% 
% References:
...
if nargin < 2 || isempty(p)
    ...
    assert(p >= 0, 'Cannot have a negative pressure.')
end
% Humidity
if nargin < 3 || isempty(h)
    ...
    assert(h >= 0 && h <= 100, 'The humidity must be in the range: 0 <= h <= 100.')
end
% Default values
xCO2        = 0.0004;
DewPoint    = false;
...
%% Basic calculations 
% Constants
R = 8.314472;                                   % [J mol^-1 K^-1]   Universal
